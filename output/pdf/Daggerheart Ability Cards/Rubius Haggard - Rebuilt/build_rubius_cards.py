...
CARDS = [
    ("01 - RAGE", "Rage"),
    ("02 - THAT ALL YOU GOT-!", "That All You Got?!"),
    ("03 - BLOODLUST", "Bloodlust"),
    ("04 - MOUNTAINOUS TENACITY", "Mountainous Tenacity"),
    ("05 - ONE WITH THE ANCIENTS", "One with the Ancients"),
    ("06 - INTENSIFYING FURY", "Intensifying Fury"),
    ("07 - STONE'S RESILIENCE", "Stone's Resilience"),
    ("08 - STRENGTH OF STONE", "Strength of Stone"),
]
...
def make_card_pdf(image_path: Path, destination: Path, title: str) -> None:
    ...
    doc.setTitle(f"Rubius Haggard — {title}")
...
def make_contact_sheet(prepared: list[tuple[Path, str]]) -> None:
    ...
    label_h = 34
    margin = 24
    sheet = Image.new("RGB", (margin * 5 + thumb_w * 4, margin * 3 + (thumb_h + label_h) * 2), "#dedbd2")
    ...
    sheet.save(QA / "Rubius Haggard - Card Proof Sheet.png", optimize=True)
...
def main() -> None:
    ...
    writer.add_metadata({
        "/Title": "Rubius Haggard — Rebuilt Ability Cards",
        "/Subject": "Eight front-only, card-sized print pages with cut edges",
        "/Author": "Daggerheart Card Creator",
    })
    with (ROOT / "Rubius Haggard - Rebuilt Ability Cards.pdf").open("wb") as output:
        writer.write(output)
