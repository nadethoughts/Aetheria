...
CARDS = [
    ("01 - CHEAT", "Cheat"),
    ("02 - VICIOUS OPPORTUNIST", "Vicious Opportunist"),
    ("03 - SNEAK ATTACK", "Sneak Attack"),
    ("04 - TENACIOUS", "Tenacious"),
    ("05 - FORMER CON ARTIST", "Former Con Artist"),
]
...
def make_contact_sheet(prepared: list[tuple[Path, str]]) -> None:
    ...
    label_h = 34
    margin = 24
    sheet = Image.new("RGB", (margin * 4 + thumb_w * 3, margin * 3 + (thumb_h + label_h) * 2), "#dedbd2")
    ...
    sheet.save(QA / "Whisper - Card Proof Sheet.png", optimize=True)
...
def main() -> None:
    ...
    writer.add_metadata({
        "/Title": "Whisper - Rebuilt Ability Cards",
        "/Subject": "Five front-only, card-sized print pages with cut edges",
        "/Author": "Daggerheart Card Creator",
    })
    with (ROOT / "Whisper - Rebuilt Ability Cards.pdf").open("wb") as output:
        writer.write(output)
