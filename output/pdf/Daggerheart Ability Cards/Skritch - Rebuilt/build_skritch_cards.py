...
CARDS = [
    ("01 - VICIOUS OPPORTUNIST", "Vicious Opportunist"),
    ("02 - SNEAK ATTACK", "Sneak Attack"),
    ("03 - LOW BLOW", "Low Blow"),
    ("04 - CHEAT", "Cheat"),
    ("05 - WILY", "Wily"),
    ("06 - SWEET TALK", "Sweet Talk"),
    ("07 - WILY UNDERDOG", "Wily Underdog"),
]
...
def make_contact_sheet(prepared: list[tuple[Path, str]]) -> None:
    ...
    sheet.save(QA / "Skritch - Card Proof Sheet.png", optimize=True)
...
def main() -> None:
    ...
    writer.add_metadata({
        "/Title": "Skritch - Rebuilt Ability Cards",
        "/Subject": "Seven front-only, card-sized print pages with cut edges",
        "/Author": "Daggerheart Card Creator",
    })
    with (ROOT / "Skritch - Rebuilt Ability Cards.pdf").open("wb") as output:
        writer.write(output)
