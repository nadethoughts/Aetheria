...
CARDS = [
    ("01 - RADIANT JUDGMENT", "Radiant Judgment"),
    ("02 - LAY ON HANDS", "Lay on Hands"),
    ("03 - ZEALOT", "Zealot"),
    ("04 - AURA OF REFUGE", "Aura of Refuge"),
    ("05 - PARAGON OF VIRTUE", "Paragon of Virtue"),
    ("06 - WELL ARMORED", "Well Armored"),
    ("07 - BUNNY LEGS", "Bunny Legs"),
]
...
def make_card_pdf(image_path: Path, destination: Path, title: str) -> None:
    ...
    doc.setTitle(f"Rowan Hearthboro - {title}")
...
def make_contact_sheet(prepared: list[tuple[Path, str]]) -> None:
    ...
    sheet.save(QA / "Rowan Hearthboro - Card Proof Sheet.png", optimize=True)
...
def main() -> None:
    ...
    writer.add_metadata({
        "/Title": "Rowan Hearthboro - Rebuilt Ability Cards",
        "/Subject": "Seven front-only, card-sized print pages with cut edges",
        "/Author": "Daggerheart Card Creator",
    })
    with (ROOT / "Rowan Hearthboro - Rebuilt Ability Cards.pdf").open("wb") as output:
        writer.write(output)
